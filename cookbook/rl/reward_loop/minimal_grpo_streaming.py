"""Minimal streaming (double-buffer) GRPO example using the reward loop."""
...
ADAPTER_NAME = os.environ.get("TWINKLE_ADAPTER_NAME", "minimal-grpo-streaming")
MAX_STEPS = int(os.environ.get("TWINKLE_MAX_STEPS", "3"))
BATCH_SIZE = int(os.environ.get("TWINKLE_BATCH_SIZE", "2"))
NUM_GENERATIONS = int(os.environ.get("TWINKLE_NUM_GENERATIONS", "2"))
# The pipeline keeps at least two batches in flight: the current one being
# submitted and the previous one being collected and trained.
...
def _make_items(prompts: list[dict[str, Any]], rows: list[dict[str, Any]], step: int) -> list[RewardItem]:
    items = []
...
async def _rollout(sampler, data_plane, prompts, step):
    ...
    if len(rows) != expected:
        ...
        raise RuntimeError(f"expected {expected} samples, got {len(rows)}")
    return rollout_ref, rows


async def _train_last(model, data_plane, pipeline, handle, last_rollout_ref):
    """Collect the previous batch (already computing in the background) and train."""
    ...
    try:
        ...
        await data_plane.arelease(last_rollout_ref)
...
async def train() -> None:
    ...
    if REWARD_BACKLOG < 2:
        raise ValueError("TWINKLE_REWARD_BACKLOG must be at least 2 for double buffering")
    ...
    try:
        ...
        last_handle = None
        last_rollout_ref = None
        for step, batch in enumerate(dataloader):
            if step >= MAX_STEPS:
                break
            prompts = [dict(row) for row in batch]
            rollout_ref, rows = await _rollout(sampler, data_plane, prompts, step)
            items = _make_items(prompts, rows, step)
            ...
            if last_handle is not None:
                # The previous batch finished in the background; only now does
                # collect block (briefly) before training it.
                reward_mean = await _train_last(model, data_plane, pipeline, last_handle, last_rollout_ref)
                print(f"step={step - 1} reward_mean={reward_mean:.4f}")

            last_handle = handle
            last_rollout_ref = rollout_ref

        # Flush the final in-flight batch.
        if last_handle is not None:
            reward_mean = await _train_last(model, data_plane, pipeline, last_handle, last_rollout_ref)
            print(f"step={MAX_STEPS - 1} reward_mean={reward_mean:.4f}")
    finally:
