...
def _sample_model_to_row(
    ...
    *,
    ...
    """Convert one generated sequence into a single row and its tag.

    Row layout matches ``_sample_models_to_rows`` so incremental writes land on
    the same global keys, where the global row index is
    ``prompt_index * num_generations + generation_idx``.  ``model`` carries a
...
async def _await_generation(
    ...
    collected = False
    try:
        async for _states in _poll_generation_events(sampler, submission_id):
            pass
...
async def _stream_sample_events(
    ...
    *,
    ...
    ``_sample_models_to_rows`` and is written in place via ``put_rows``), then
    a ``ref`` line with the fully-filled DataRef.  On failure it cancels the
    generation, releases the ref and yields an ``error`` line.
    """
    written: set[int] = set()
    try:
        async for states in _poll_generation_events(sampler, submission_id):
            ...
            models = _responses_to_models([response for _, response in responses])
...
def _register_twinkle_sampler_routes(app: FastAPI, self_fn: Callable[[], SamplerManagement]) -> None:
    ...
    async def sample_to_data_plane_stream(
        ...
        Lines:
          ...
          per completed sequence (row layout matches ``_sample_models_to_rows``
