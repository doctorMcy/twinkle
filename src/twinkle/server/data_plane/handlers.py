...
def register_data_plane_routes(app: FastAPI, self_fn: Callable[[], 'DataPlaneManagement']) -> None:
    ...
    async def put(body: types.DataPutRequest,
    ...
    async def get(body: types.DataGetRequest,
        ...
        tags = (
            await self.store.get_tags(body.ref)
            if body.include_tags else []
        )
        return types.DataRowsResponse(rows=rows, tags=tags)

    @app.post('/twinkle/append', response_model=types.DataRef)
    async def append(body: types.DataAppendRequest,
                     self: DataPlaneManagement = Depends(self_fn)) -> types.DataRef:
    ...
    async def release(body: types.DataReleaseRequest,
                      self: DataPlaneManagement = Depends(self_fn)) -> dict[str, str]:
