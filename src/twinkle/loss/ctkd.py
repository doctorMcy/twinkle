...
class CTKDLoss(Loss):
    """Cross-Tokenizer Knowledge Distillation Loss using X-Token projection matrix.

    This loss enables knowledge distillation between models with different tokenizers
    by constructing sparse projection matrices W that align student and teacher
    vocabulary spaces for multiple teachers.

    The projection matrix construction follows three steps:
    1. Initialize W[s,t] = 0 for all student token s and teacher token t
    2. Exact match: If student token text equals teacher token text, set W[s,t] = 1
    3. Multi-token decoding: For unmatched student tokens, decode to text and
       re-encode with teacher tokenizer. If the resulting sequence length < L (max_length),
       assign weights: W[s, t[i]] = β * γ^i
    ...
    def __init__(
        ...
        if teacher_weights is None:
            ...
            if len(teacher_weights) != self.num_teachers:
                raise ValueError(
                    f"Number of weights ({len(teacher_weights)}) must match number of teachers ({self.num_teachers})")
        ...
        # Auto-detect NPU device if not specified
        if device is None:
            if torch.cuda.is_available():
                self.device = torch.device('cuda')
            elif hasattr(torch, 'npu') and torch.npu.is_available():
                self.device = torch.device('npu:0')
                print(f"Auto-detected NPU device: {self.device}")
            else:
    ...
    def _ensure_projection_matrices_built(self):
        ...
        if self._projection_matrices_built:
            return
        # Use a lock to ensure only one thread builds the projection matrices
        ...
        with self._build_lock:
            ...
            if cache_key in _PROJECTION_MATRIX_CACHE:
                ...
                self.projection_student_indices_list = [t.to(self.device) if self.device is not None else t.clone() for
                                                        t in cached_data['projection_student_indices_list']]
                self.projection_teacher_indices_list = [t.to(self.device) if self.device is not None else t.clone() for
                                                        t in cached_data['projection_teacher_indices_list']]
                self.projection_values_list = [t.to(self.device) if self.device is not None else t.clone() for t in
                                               cached_data['projection_values_list']]
            else:
            ...
            if self.loss_type == 'hkl':
                hkl_start_time = time.perf_counter()
    ...
    def __call__(
        ...
        if teacher_logits_group is None and teacher_topk_logprobs_group is not None and teacher_topk_indices_group is not None:
            if len(teacher_topk_logprobs_group) != self.num_teachers or len(
                    teacher_topk_indices_group) != self.num_teachers:
                raise ValueError(
                    f"Number of teachers in topk format ({len(teacher_topk_logprobs_group)}) must match number of teachers ({self.num_teachers})")
            ...
            for i in range(self.num_teachers):
                ...
                # CRITICAL FIX: vLLM returns logprobs (log probabilities), not logits.
                # We convert logprobs to probabilities using exp().
                # The resulting tensor represents teacher probabilities, not logits.
                # These will be used directly as probabilities in _compute_pkl_loss/_compute_hkl_loss
                # (NOT applying softmax again).

                # Create full probability tensor initialized with zeros
                ...
                # Scatter the topk probabilities into the full probability tensor
                ...
                # Store as logits_group for compatibility, but these are actually probabilities
                # The _compute_pkl_loss/_compute_hkl_loss functions will use them directly
        ...
        if len(teacher_logits_group) != self.num_teachers:
            raise ValueError(
                f"Number of teacher logits ({len(teacher_logits_group)}) must match number of teachers ({self.num_teachers})")

        # Get labels - prefer teacher_labels from kwargs for CTKD loss mask
        # The student labels may have -100 for most positions (e.g., only response tokens have valid labels),
        # but for CTKD we want to compute loss over all positions where the teacher has valid predictions.
        ...
        # Use teacher_labels for num_tokens calculation if available
        if teacher_labels_group is not None and len(teacher_labels_group) > 0:
            num_tokens = teacher_labels_group[0].ne(-100).sum().item()
        else:
            num_tokens = labels.ne(-100).sum().item() if labels is not None else 0
        loss_output = LossOutput(loss=total_loss, num_tokens=num_tokens)
        return loss_output
    ...
    def _build_projection_matrix_for_teacher(self, teacher_tokenizer, teacher_index):
        ...
        # Convert to tensors directly on target device to avoid device transfers
        ...
        values_tensor = torch.tensor(values, dtype=torch.float32,
                                     device=self.device)  # Use float32 for better gradient computation
        ...
        sparse_memory_mb = (student_indices_tensor.numel() * 8 +
                            ...
                            values_tensor.numel() * 4) / (1024 ** 2)

        print(
            f"Built projection matrix for teacher {teacher_index}: {len(student_indices)} mappings, memory: {sparse_memory_mb:.2f} MB")
    ...
    def _compute_pkl_loss(
            ...
            teacher_probs: torch.Tensor,  # NOTE: These are already probabilities (from exp(logprobs)), NOT logits
            labels: torch.Tensor,
            teacher_index: int = 0,
    ) -> torch.Tensor:
        """
        ...
        CRITICAL: The `teacher_probs` input is expected to be already in probability space
        (converted from vLLM logprobs via exp()). NO softmax should be applied to it.

        Args:
            student_logits: [batch, seq_len, student_vocab_size] - raw logits from student model
            teacher_probs: [batch, seq_len, teacher_vocab_size] - probabilities from teacher model
        ...
        # Compute student probabilities with temperature
        student_probs = F.softmax(shift_student_logits / self.temperature, dim=-1)

        # ============ DEBUG LOGS ============
        print(f"[CTKD DEBUG] _compute_pkl_loss: student_logits shape: {student_logits.shape}")
        print(f"[CTKD DEBUG] _compute_pkl_loss: teacher_probs shape: {teacher_probs.shape}")
        print(f"[CTKD DEBUG] _compute_pkl_loss: labels shape: {labels.shape}")
        print(f"[CTKD DEBUG] _compute_pkl_loss: shift_student_logits shape: {shift_student_logits.shape}")
        print(f"[CTKD DEBUG] _compute_pkl_loss: shift_teacher_probs shape: {shift_teacher_probs.shape}")
        print(f"[CTKD DEBUG] _compute_pkl_loss: shift_labels shape: {shift_labels.shape}")
        print(f"[CTKD DEBUG] _compute_pkl_loss: loss_mask sum: {loss_mask.sum().item()}, total: {loss_mask.numel()}")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: student_logits range: [{shift_student_logits.min().item():.4f}, {shift_student_logits.max().item():.4f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: student_probs range: [{student_probs.min().item():.6f}, {student_probs.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: student_probs sum per position: [{student_probs.sum(dim=-1).min().item():.4f}, {student_probs.sum(dim=-1).max().item():.4f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: teacher_probs range: [{shift_teacher_probs.min().item():.6f}, {shift_teacher_probs.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: teacher_probs sum per position: [{shift_teacher_probs.sum(dim=-1).min().item():.6f}, {shift_teacher_probs.sum(dim=-1).max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: teacher_nonzero_count per position: [{(shift_teacher_probs > 0).sum(dim=-1).min().item()}, {(shift_teacher_probs > 0).sum(dim=-1).max().item()}]")
        print(f"[CTKD DEBUG] _compute_pkl_loss: temperature: {self.temperature}")
        # ====================================

        # CRITICAL: teacher_probs are already probabilities (converted from vLLM logprobs via exp()).
        # They are a top-k truncated distribution - most entries are 0.
        # We should NOT apply softmax or temperature scaling to them.
        # The teacher distribution is used as-is.
        ...
        shift_teacher_probs = shift_teacher_probs[:, :min_seq_len, :]
        ...
        _, _, teacher_vocab_size = shift_teacher_probs.shape
        ...
        # CRITICAL FIX: Do NOT renormalize projected_student_probs.
        # The projection matrix W[s,t] is designed so that Σ_t W[s,t] = 1 for each student token s,
        # and the scatter_add operation preserves the total probability mass.
        # Renormalization would destroy the probability structure and inflate the loss.
        # The KL divergence will only be computed over positions where teacher has non-zero probability,
        # so the missing probability mass in teacher's top-k truncated distribution is handled correctly.

        # Debug: Check dimensions before KL divergence
        if projected_student_probs.size(-1) != shift_teacher_probs.size(-1):
            raise ValueError(
                f"Vocabulary dimension mismatch: projected_student_probs has size {projected_student_probs.size(-1)}, "
                f"teacher_probs has size {shift_teacher_probs.size(-1)}. "
                f"Projection matrix was built for teacher vocab size {self.teacher_vocab_size}, "
                f"but actual teacher model has vocab size {shift_teacher_probs.size(-1)}. "
        ...
        teacher_nonzero_mask = shift_teacher_probs > 0  # [batch, seq_len, teacher_vocab]

        # Compute KL divergence only over teacher non-zero positions
        # KL(P||Q) = Σ P(x) * log(P(x)/Q(x))
        # We compute this manually to handle the masking correctly
        log_projected_student = torch.log(projected_student_probs + 1e-8)

        # Element-wise KL contribution: P * log(P/Q)
        # Only compute for positions where teacher has non-zero probability
        kl_contrib = shift_teacher_probs * (
                torch.log(shift_teacher_probs + 1e-8) - log_projected_student
        )

        # Zero out contributions from teacher-zero positions
        ...
        # ============ DEBUG LOGS: KL Computation ============
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: projected_student_probs range: [{projected_student_probs.min().item():.6f}, {projected_student_probs.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: projected_student_probs sum per position: [{projected_student_probs.sum(dim=-1).min().item():.6f}, {projected_student_probs.sum(dim=-1).max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: log_projected_student range: [{log_projected_student.min().item():.4f}, {log_projected_student.max().item():.4f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: kl_contrib range: [{kl_contrib.min().item():.6f}, {kl_contrib.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: kl_div range (per position): [{kl_div.min().item():.4f}, {kl_div.max().item():.4f}]")
        print(f"[CTKD DEBUG] _compute_pkl_loss: kl_div mean (unmasked): {kl_div.mean().item():.4f}")
        print(
            f"[CTKD DEBUG] _compute_pkl_loss: masked_kl range: [{masked_kl.min().item():.4f}, {masked_kl.max().item():.4f}]")
        print(f"[CTKD DEBUG] _compute_pkl_loss: final loss: {loss.item():.6f}")

        # Check for potential issues
        if projected_student_probs.max().item() < 1e-6:
            print(f"[CTKD WARNING] projected_student_probs is nearly zero! This indicates projection matrix issues.")
        if kl_div.max().item() > 50:
            print(f"[CTKD WARNING] Very high KL divergence detected! Max kl_div: {kl_div.max().item():.4f}")
        # ====================================
    ...
    def _compute_hkl_loss(
        ...
        # ============ DEBUG LOGS ============
        print(f"[CTKD DEBUG] _compute_hkl_loss: student_logits shape: {student_logits.shape}")
        print(f"[CTKD DEBUG] _compute_hkl_loss: teacher_probs shape: {teacher_probs.shape}")
        print(f"[CTKD DEBUG] _compute_hkl_loss: labels shape: {labels.shape}")
        print(f"[CTKD DEBUG] _compute_hkl_loss: shift_student_logits shape: {shift_student_logits.shape}")
        print(f"[CTKD DEBUG] _compute_hkl_loss: shift_teacher_probs shape: {shift_teacher_probs.shape}")
        print(f"[CTKD DEBUG] _compute_hkl_loss: shift_labels shape: {shift_labels.shape}")
        print(f"[CTKD DEBUG] _compute_hkl_loss: loss_mask sum: {loss_mask.sum().item()}, total: {loss_mask.numel()}")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: student_logits range: [{shift_student_logits.min().item():.4f}, {shift_student_logits.max().item():.4f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: student_probs range: [{student_probs.min().item():.6f}, {student_probs.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: student_probs sum per position: [{student_probs.sum(dim=-1).min().item():.4f}, {student_probs.sum(dim=-1).max().item():.4f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: teacher_probs range: [{shift_teacher_probs.min().item():.6f}, {shift_teacher_probs.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: teacher_probs sum per position: [{shift_teacher_probs.sum(dim=-1).min().item():.6f}, {shift_teacher_probs.sum(dim=-1).max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: teacher_nonzero_count per position: [{(shift_teacher_probs > 0).sum(dim=-1).min().item()}, {(shift_teacher_probs > 0).sum(dim=-1).max().item()}]")
        print(f"[CTKD DEBUG] _compute_hkl_loss: temperature: {self.temperature}")
        # ====================================
        student_seq_len = student_probs.shape[1]
        teacher_seq_len = shift_teacher_probs.shape[1]
        min_seq_len = min(student_seq_len, teacher_seq_len)
        ...
        for i in range(0, len(valid_student_ids), chunk_size):
            ...
            mapped_student_probs.scatter_add_(
                dim=2,
                index=chunk_teacher_ids.unsqueeze(0).unsqueeze(0).expand(batch_size, seq_len, -1).to(
                    mapped_student_probs.device),
                src=student_probs[:, :, chunk_student_ids].to(mapped_student_probs.device)
            )

        # CRITICAL FIX: Do NOT renormalize mapped_student_probs.
        # The mapping preserves probability mass only for tokens with valid mappings.
        # Renormalization would destroy the probability structure.

        # ============ DEBUG LOGS: Mapping Statistics ============
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: best_mapping valid count: {valid_mask.sum().item()} / {best_mapping.numel()}")
        print(f"[CTKD DEBUG] _compute_hkl_loss: valid_student_ids count: {len(valid_student_ids)}")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: mapped_student_probs range: [{mapped_student_probs.min().item():.6f}, {mapped_student_probs.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: mapped_student_probs sum per position: [{mapped_student_probs.sum(dim=-1).min().item():.6f}, {mapped_student_probs.sum(dim=-1).max().item():.6f}]")
        # ====================================

        # CRITICAL FIX: Compute KL divergence only over positions where teacher has non-zero probability.
        teacher_nonzero_mask = shift_teacher_probs > 0  # [batch, seq_len, teacher_vocab]
        ...
        # Element-wise KL contribution: P * log(P/Q)
        kl_contrib = shift_teacher_probs * (
                torch.log(shift_teacher_probs + 1e-8) - log_mapped_student
        )
        # Zero out contributions from teacher-zero positions
        kl_contrib = kl_contrib * teacher_nonzero_mask
        ...
        # ============ DEBUG LOGS: KL Computation ============
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: log_mapped_student range: [{log_mapped_student.min().item():.4f}, {log_mapped_student.max().item():.4f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: kl_contrib range: [{kl_contrib.min().item():.6f}, {kl_contrib.max().item():.6f}]")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: kl_div range (per position): [{kl_div.min().item():.4f}, {kl_div.max().item():.4f}]")
        print(f"[CTKD DEBUG] _compute_hkl_loss: kl_div mean (unmasked): {kl_div.mean().item():.4f}")
        print(
            f"[CTKD DEBUG] _compute_hkl_loss: masked_kl range: [{masked_kl.min().item():.4f}, {masked_kl.max().item():.4f}]")
        print(f"[CTKD DEBUG] _compute_hkl_loss: final loss: {loss.item():.6f}")

        # Check for potential issues
        if mapped_student_probs.max().item() < 1e-6:
            print(f"[CTKD WARNING] mapped_student_probs is nearly zero! This indicates mapping issues.")
        if kl_div.max().item() > 50:
            print(f"[CTKD WARNING] Very high KL divergence detected! Max kl_div: {kl_div.max().item():.4f}")
        # ====================================
